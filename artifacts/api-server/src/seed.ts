...
async function seed() {
  ...
  await db.insert(subscriptionPlansTable).values([
    ...
    },
    {
      ...
      name: "Mofk",
      nameAr: "مفك",
      descriptionAr: "اشتراك مفك لمركبة واحدة مع توصيات ذكية",
      priceMonthlySar: 29,
      priceYearlySar: 290,
      maxVehicles: 1,
      tier: "plus",
      features: ["1 vehicle", "AI assistant", "PDF export"],
      featuresAr: [
        "مركبة واحدة فقط",
    ...
    },
    {
      ...
      name: "Family",
      nameAr: "العائلة",
      descriptionAr: "لعدة مركبات عائلية مع تصدير Excel",
      priceMonthlySar: 59,
      priceYearlySar: 590,
      maxVehicles: 3,
      tier: "family",
      features: ["3 vehicles", "AI assistant", "Excel export"],
      featuresAr: [
        "حتى 3 مركبات",
        "تشخيص متقدم وتفسير عربي مفصّل",
        "المساعد الذكي بالعربية",
  ...
  const otherUsers = arabicNames.map((name, i) => ({
    ...
    city: pick(cities),
    role: "user",
    subscriptionTier: pick(["free", "free", "plus", "family", "free", "fleet"]),
